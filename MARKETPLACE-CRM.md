...
**Importante sobre a fonte de dados de tendências**: não existe uma API pública e gratuita oficial do Google Trends (a biblioteca não-oficial mais usada, `pytrends`, foi arquivada em 2025). Por padrão este app usa `CuratedTrendsProvider`, um dataset de referência curado à mão (`server/src/data/trendSignals.ts`) para termos de beleza/perfumaria — sinalizado como `source: "curado"` no resultado, para deixar claro que não é dado ao vivo. Para dados **reais**, crie uma conta grátis em [serpapi.com](https://serpapi.com) e cole a chave no campo "Chave da SerpApi" da tela de **Configurações** (ou na variável `SERPAPI_KEY` do `.env` — o valor salvo em Configurações tem prioridade). Com a chave configurada, o app passa a buscar interesse de busca real via `SerpApiTrendsProvider`, com fallback automático para o dataset curado em qualquer falha (chave inválida, limite de requisições, erro de rede — mesmo padrão do `ResilientAIProvider`); o campo `trendsProvider` no resultado sempre diz qual dos dois realmente respondeu.
...
- **Interesse de busca por região**: quando a chave da SerpApi está configurada, isso é dado real do Google Trends (`data_type=GEO_MAP_0`, "interest by region"), agregado por estado brasileiro em macro-região (`server/src/data/regionalReference.ts#STATE_TO_REGION`). Sem a chave, é uma variação estimada e claramente sinalizada (`source: "curado (sem repartição real por região)"`).
